...
export default class Comments extends React.Component{
    ...
    getCommentsData(){
        ProductAjax.getComment(global.productId,this.state.pageNum,this.state.pageSize).then((result) => {
            ...
            this.setState({
                comments:this.state.comments.concat(resObj.data.list)
            })
        }).catch((err) => {
    ...
    render(){
        return(
            ...
            <View style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}><Text style={{color:'blue',borderBottomColor:'gray',borderBottomWidth:1,borderStyle:'solid'}}>加载更多</Text></View>
            </TouchableOpacity>
                </ScrollView>
            </View>
        )
